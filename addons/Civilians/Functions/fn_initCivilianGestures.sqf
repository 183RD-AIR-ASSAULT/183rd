...
if !(isNil "OETA_Civilians_CIGSInitialized") exitWith {};

OETA_Civilians_CIGSInitialized = true;

//["OETA_Civilians_"] call CBA_settings_fnc_get;
...
["ace_common_playActionNow", {
    ...
    if !(OETA_Civilians_enableGestures) exitWith {};

    // The percentage chance a civilian will listen
    private _chance = [OETA_Civilians_successChance_armed, OETA_Civilians_successChance_unarmed] select (count weapons _player > 0);
...
// Commands affecting units within angle sector
    ...
    if ((OETA_Civilians_enableStopGestures) && ({_x == _gesture} count _acceptedGestures > 0)) then {
        {
                ...
                if (count weapons _x == 0 && {random 1 < _chance}) then {
                    if ([position _player, getDir _player, 30, position _x] call BIS_fnc_inAngleSector) then {
                        if (vehicle _x == _x) then {
                            ...
                            _x setVariable ["oeta_is_following", nil, true];
    ...
    if ((OETA_Civilians_enableGoAwayGestures) && ({_x == _gesture} count _acceptedGestures > 0)) then {
    ...
    if ((OETA_Civilians_enableGetDownGestures) && ({_x == _gesture} count _acceptedGestures > 0)) then {
        // Source: https://github.com/acemod/ACE3/blob/master/addons/interaction/functions/fnc_sendAway.sqf
        // Extracted from source to avoid an infinite loop caused by line 23 in source
        {
            if (count weapons _x == 0 && {random 1 < _chance}) then {
...
// Commands affecting units only under cursor
    ...
    if ((OETA_Civilians_enableFollowGestures) && ({_x == _gesture} count _acceptedGestures > 0) && (_player distance _target < 10)) then {
        ...
        [format["%1 told %2 to follow using a %3 gesture", _player, _target, _gesture]] call YAINA_F_fnc_log;
            ...
            private _following = [_target, _player] spawn {
                ...
                _target setVariable ["oeta_is_following", _player, true];

                [format["%1 about to move to %2 (%3)", _target, _player, _target getVariable ["oeta_is_following", "nothing"]]] call YAINA_F_fnc_log;
                private _playerPosition = [];
                private _index = 0;

                while {(_target getVariable ["oeta_is_following", false]) isEqualTo _player} do {
                    if (_index > 30) exitWith {
                        _target setVariable ["oeta_is_following", nil, true];
                    };
    ...
    if ((OETA_Civilians_enableGreetingGestures) && ({_x == _gesture} count _acceptedGestures > 0)) then {
        if (
