...
{
    _Generator = _x;
    _Generator addAction ["Power ON",{(_this select 3) execVM "\z\oeta\addons\zeus\functions\fnc_TogglePower.sqf";},[_logic,"AUTO", _triggers],1.5,true,false,"","true",3];
    _Generator addAction ["Power OFF",{(_this select 3) execVM "\z\oeta\addons\zeus\functions\fnc_TogglePower.sqf";},[_logic,"OFF", _triggers],1.5,true,false,"","true",3];
    ...
    _Generator addEventHandler["Killed", {
        ...
        if((_logic getVariable ["GRID_POWER","OFF"]) == "OFF") then {
            [_logic,"OFF", _triggers] execVM  "\z\oeta\addons\zeus\functions\fnc_TogglePower.sqf";
        };
    }];
    /*
...
[_logic,_logic getVariable ["GRID_POWER","AUTO"], _triggers] execVM "\z\oeta\addons\zeus\functions\fnc_TogglePower.sqf";
...
["GRID_CHANGE",{
    ...
    if (_GridPower == "AUTO") then {
        ...
        if ((_logic getVariable ["POWER", "OFF"]) == "OFF" ) exitWith {};
        // time to turn on
        [_logic,"AUTO",_triggers] execVM "\z\oeta\addons\zeus\functions\fnc_TogglePower.sqf";
    };
