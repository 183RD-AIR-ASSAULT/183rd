/*
Function: oeta_fnc_ConfigZeus

Description:
    Used to configure a Zeus Module. Ensures that any AI placed down by that Zeus have Dynamic Sim
    enabled and their difficulty settings configured to the OETA Default.
    ...
    "CuratorObjectPlaced", {
        ...
        _result  = call {
            if (_class isKindOf "Man") exitWith {
                ...
                [_Group, "OETA Default"] remoteExec ["oeta_fnc_SetUnitSkill",2];
                [_Group, true] remoteExec ["enableDynamicSimulation",2];
            };

            if (_class isKindOf "AllVehicles") exitWith { {
                    [_x, "OETA Default"] remoteExec ["oeta_fnc_SetUnitSkill",2];
