 ...
 * \oeta_core\functions\sounds\fn_loop3dSoundLocal.sqf
 * by Ojemineh
 *
 ...
 *
 * Example:
 * [player, "AlarmBell", [], -1, -1, 6] call oeta_core_fnc_loop3dSoundLocal;
 *
 */
...
if (_distance <= 0) then {_distance = [_soundClass] call oeta_core_fnc_getSoundDistance;};
if (_distance <= 0) then {_distance = 100;};
//if ((_maxDistance > 0) && ((player distance _object) > _maxDistance)) exitWith {};

if (_duration <= 0) then {_duration = [_soundClass] call oeta_core_fnc_getSoundDuration;};
if (_duration <= 0) exitWith {};

if (_pitch <= 0) then {_pitch = [_soundClass] call oeta_core_fnc_getSoundPitch;};
if (_pitch <= 0) then {_pitch = 1;};

// -------------------------------------------------------------------------------------------------

private _soundArray = _object getVariable ["OETA_sound3d", []];
...
if (_soundInUse) exitWith {
    [_object, _soundClass, _position, _distance, _maxDistance, _duration, _pitch, _isSpeech] spawn {
        ...
        [_object, _soundClass] call oeta_core_fnc_stop3dSoundLocal;
        sleep 0.1;
        [_object, _soundClass, _position, _distance, _maxDistance, _duration, _pitch, _isSpeech] call oeta_core_fnc_loop3dSoundLocal;
    };
};

// -------------------------------------------------------------------------------------------------

private _soundObject = createSimpleObject [oeta_core_HELPER_OBJECT, [0,0,0], true];
_soundObject hideObject true;
_soundObject attachTo [_object, _position];

_soundArray pushBackUnique [_soundClass, _soundObject];
_object setVariable ["OETA_sound3d", _soundArray];
...
private _handler = [_object, _soundObject, _soundClass, _distance, _maxDistance, _duration, _pitch, _isSpeech] spawn {
    ...
    while {(alive _object) && !(isNull _soundObject)} do {
        [_soundObject, _soundClass, _distance, _maxDistance, _pitch, _isSpeech] call oeta_core_fnc_say3dLocal;
