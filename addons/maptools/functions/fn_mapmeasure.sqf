...
findDisplay 12 displayaddEventHandler ["MouseButtonDown",
    {if (_this select 5) then
        ...
        player setVariable [ "oeta_mapmeasurer_start",((findDisplay 12) displayCtrl 51) ctrlMapScreenToWorld [(_this select 2), (_this select 3)]];
        player setVariable ["oeta_mapmeasurere_myDistance",0];
        ...
        _id_MouseMoving = ((findDisplay 12) displayCtrl 51) ctrladdEventHandler ["MouseMoving",
            {_myActualCoordinates = ((findDisplay 12) displayCtrl 51) ctrlMapScreenToWorld [(_this select 1),(_this select 2)];
                ...
                _myDistance = player getVariable "oeta_mapmeasurere_myDistance";
                _myStartCoordinates = player getVariable "oeta_mapmeasurer_start";
                _myDistance =  (_myActualCoordinates distance2D _myStartCoordinates) + _myDistance;
            player setVariable [ "oeta_mapmeasurer_start",_myActualCoordinates];
            player setVariable ["oeta_mapmeasurere_myDistance",_myDistance];
            }
            ];
            player setVariable ["oeta_mapmeasurer_id_MouseMoving",_id_MouseMoving];
        }
    }
    ];
};
...
findDisplay 12 displayaddEventHandler ["MouseButtonUp",
    {if ((isnil str (player getVariable "oeta_mapmeasurere_myDistance")) && ((player getVariable "oeta_mapmeasurere_myDistance") > 1)) then
        {
            private ["_displaytext","_id_MouseMoving"];
            _displaytext =  format ["Distance: %1 meters",round (player getVariable "oeta_mapmeasurere_myDistance")];
            ...
            player setVariable ["oeta_mapmeasurere_myDistance", nil];
            player setVariable ["oeta_mapmeasurer_start", nil];
            _id_MouseMoving = player getVariable "oeta_mapmeasurer_id_MouseMoving";
