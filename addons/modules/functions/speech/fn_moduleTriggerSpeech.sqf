 ...
 * \z\oeta\addons\modules\functions\speech\fn_moduleTriggerSpeech.sqf
 * by Ojemineh
 *
 * module function
 *
 ...
 *
 * Example:
 * [] call OETA_fnc_moduleTriggerSpeech;
 *
 */
...
switch _mode do {
    ...
    case "init": {
        ...
        if (_source isEqualTo "") exitWith {
            [format [localize "STR_OETA_ModuleError_UnitIsEmpty", _source]] call BIS_fnc_error;
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_UnitIsEmpty", _source]], "modules"] call OETA_common_fnc_log;
        };
        ...
        if (isNull (missionNamespace getVariable [_source, objNull])) exitWith {
            ...
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_VarIsUnknown", _source]], "modules"] call OETA_common_fnc_log;
        };

        private _unit = missionNamespace getVariable _source;

        if !(_unit isKindOf "Man") exitWith {
            [format [localize "STR_OETA_ModuleError_IsNotUnit", _unit]] call BIS_fnc_error;
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_IsNotUnit", _unit]], "modules"] call OETA_common_fnc_log;
        };
        ...
        if (_soundExist isEqualTo 0) exitWith {
            ...
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_UnknownInCfgSounds", _sound]], "modules"] call OETA_common_fnc_log;
        };
        ...
        if (_distance <= 0) exitWith {
            ...
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_DistanceGreaterZero", _distance]], "modules"] call OETA_common_fnc_log;
        };
        ...
        if ((_maxDistance > 0) && (_maxDistance <= _distance)) exitWith {
            ...
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_MaxGreaterDistance", _distance, _maxDistance]], "modules"] call OETA_common_fnc_log;
        };
        ...
        if (_duration <= 0) exitWith {
            ...
            [2, "ModuleTriggerpeech '%1' - %2", [_logic, format [localize "STR_OETA_ModuleError_DurationNotDefined", _sound]], "modules"] call OETA_common_fnc_log;
        };
        ...
        if (_isActivated) then {
            ...
            if (_loop) then {
                ...
                private _pfhHandler = [{
                    ...
                    if (!alive _logic) exitWith {
                        [_unit, _sound] call OETA_fnc_stop3dSound;
                    ...
                    if ((CBA_missionTime - _lastTimePlayed) >= _duration) then {
                        ...
                        [_unit, _sound, _distance, _maxDistance, _duration] call OETA_fnc_speak3d;
                ...
                _logic setVariable ["oeta_pfhHandler", _pfhHandler];
            ...
            if (_animation != "") then {
                _unit setVariable ["oeta_isAnimEnabled", (_unit checkAIFeature "ANIM")];
                _unit setVariable ["oeta_isMoveEnabled", (_unit checkAIFeature "MOVE")];
                _unit setVariable ["oeta_unitPosition",  (getPosASL _unit)];
                _unit setVariable ["oeta_unitDirection", (getDir _unit)];
            ...
            if !(_ignoreStop) then {
                ...
                if (_loop) then {
                    private _pfhHandler = _logic getVariable ["oeta_pfhHandler", -1];
                    if (_pfhHandler > -1) then {[_pfhHandler] call CBA_fnc_removePerFrameHandler;};
                    _logic setVariable ["oeta_pfhHandler", nil];
                };

                [_unit, _sound] call OETA_fnc_stop3dSound;

                if (_animation != "") then {
                    private _isAnimEnabled = _unit getVariable ["oeta_isAnimEnabled", true];
                    private _isMoveEnabled = _unit getVariable ["oeta_isMoveEnabled", true];
                    private _unitPosition  = _unit getVariable ["oeta_unitPosition",  (getPosASL _unit)];
                    private _unitDirection = _unit getVariable ["oeta_unitDirection", (getDir _unit)];
