 ...
 * \z\oeta\addons\modules\functions\speech\fn_moduleGlobalSpeechPlay.sqf
 * by Ojemineh
 *
 * module function
 *
 ...
 *
 * Example:
 * [] call OETA_fnc_moduleGlobalSpeechPlay;
 *
 */
...
[_logic, _caller, _target, _source, _sound, _distance, _maxDistance, _duration, _onlyOnce, _reaction, _animation, _codeStart, _codeEnd] spawn {
    ...
    if (_onlyOnce) then {_target setVariable ["oeta_playedOnce", true, true];};
    ...
    [_target, _sound, _distance, _maxDistance, _duration, 1, true] call OETA_fnc_speak3d;
    ...
    waitUntil {if (!(_target getVariable ["oeta_speak3d", false])) exitWith {true}; false};
