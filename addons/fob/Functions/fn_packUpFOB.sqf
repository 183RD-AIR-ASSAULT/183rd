...
if (!isServer) exitWith {
    _this remoteExec ["OETA_fnc_packUpFOB", 2];
};
...
{
    deleteVehicle _x;
} forEach (_object getVariable ["OETA_PortableFOB_Objects", []]);

// remove respawn position
_respawnPos = _object getVariable "OETA_PortableFOB_RespawnPos";
...
} forEach (_object getVariable ["OETA_PortableFOB_hiddenObjects", []]);
