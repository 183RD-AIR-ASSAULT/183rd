...
// Message admins
private _message = format["[oeta] Ending mission..."];
...
if (!isNil "ocap_fnc_exportData") then {
    _message = format["[oeta] OCAP detected exporting (10 second timeout)..."];
    [_message,'tac1_admin_fnc_messageAdmin',true] call BIS_fnc_MP;
    ...
    [] spawn {
         {
            ...
            private _message = format["[oeta] OCAP Export complete. Ending..."];
            [_message,'tac1_admin_fnc_messageAdmin',true] call BIS_fnc_MP;
            GVAR(endMissionWait) = -1;
