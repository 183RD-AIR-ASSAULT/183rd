...
uiNamespace setVariable ["oeta_assignGear_validFaces",_faceClasses];

{
    ...
    if (count _weightedArray > 0) then {
        uiNamespace setVariable ["oeta_assignGear_faceset_"+_name,_weightedArray];
    };
} forEach ("true" configClasses (configFile >> "oeta_assignGear_facesets"));
