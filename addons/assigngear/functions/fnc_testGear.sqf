...
private _fncTestUnit = {
    ...
    if (isClass _config) then {
        ...
        private _validFaces = uiNamespace getVariable ["oeta_assignGear_validFaces",[]];

        {
            private _face = toLower _x;
             if ((_face find "faceset:") isEqualTo 0) then {
                private _facesetName = _face select [8];
                private _array = uiNamespace getVariable ["oeta_assignGear_faceset_" + _facesetName,0];
...
{
    private _unit = _x;
    (_unit get3DENAttribute 'OETA_assignGear_enabled') params [["_enabled",false]];

    if (_enabled) then {
        (_unit get3DENAttribute 'OETA_assignGear_role') params [["_role","r"]];
        (_unit get3DENAttribute 'OETA_assignGear_faction') params [["_faction",toLower(faction _unit)]];
