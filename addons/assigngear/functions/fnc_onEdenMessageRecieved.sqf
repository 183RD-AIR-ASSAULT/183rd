...
/*
 * Name = OETA_assignGear_fnc_onEdenMessageRecieved
 * Author = Snippers
 *
 * Arguments:
...
if (_messageId isEqualTo 0) then { //Mission saved.
    ...
    if FILE_EXISTS("description.ext") then {
        ...
        if (!(_newdescription isEqualTo GVAR(descriptionExt))) then {
            ...
            {
                (_x get3DENAttribute "OETA_assignGear_full") params ["_value"];
                [_x,_value] call oeta_assignGear_fnc_helper;
            } forEach (allUnits);
            GVAR(descriptionExt) = _newdescription;
            ["Scenario Saved - description.ext changed, reapplying OETA gear"] call BIS_fnc_3DENNotification;
            _return = true;
        };
    } else {
        GVAR(descriptionExt) = "";
