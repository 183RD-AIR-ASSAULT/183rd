/*
Function: OETA_fnc_initInspectableIntel
...
Examples:
    ...
    ] call OETA_fnc_initInspectableIntel;
    (end)
...
//    testing, sharing with side and one specific OETA group works though)
...
// Add the action
[_object, _actionTitle, _diaryRecord select 0] remoteExec ["OETA_fnc_SetIntelAction",0,true];
...
if (typeName _sharedWith == "STRING") then {
    _recipients = []; {
        if (side _x == west && {_x getVariable ["OETA_section", "Command"] == _sharedWith}) then {
...
if (_notifySide) then {
    [
        ...
        "IntelObjectFound", {
            params[ "", "_foundBy" ];
            [format ["Intel collected by %1", name _foundBy], [1, -0.2], "#339900", 0.5] call OETA_fnc_dynamicText;
        }
