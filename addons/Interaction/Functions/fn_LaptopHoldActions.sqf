/*
Function: OETA_fnc_LaptopHoldActions
...
Examples:
    (begin example)
        [this] call OETA_fnc_LaptopHoldActions;
        [MyLaptopName] call OETA_fnc_LaptopHoldActions;
        [this, "Download the Intel"] call OETA_fnc_LaptopHoldActions;
        [this, "Trigger the Alarm", 5, "A3\Sounds_F\sfx\alarm_blufor.wss"] call OETA_fnc_LaptopHoldActions;
    (end)
...
[
    ...
    "true",
    "true", { [format["<t color='#FFBB00' size = '.5'>Processing</t>",(_this select 2) select 1],-1,0.8,(_this select 2) select 2,2,0,789] spawn BIS_fnc_dynamicText;}, {}, {
        ...
        _Newobject = createVehicle ["Land_Laptop_F", [0,0,0], [], 0,"NONE"]; {[_Newobject, false] remoteExec [_x, 2];} foreach ["enableSimulationGlobal","allowDamage"];
        _Newobject SetPosATL _OriginalPos;
        _Newobject setDir _OriginalDir; {_x addCuratorEditableObjects [[_Newobject], false];} forEach allcurators;
//		playSound3D [((_this select 2) select 3), _Newobject, false, getposATL _Newobject, 1, 1, 200];
        [_Newobject, (_this select 2) select 3, (_this select 2) select 4, (_this select 2) select 5] call OETA_fnc_Sound3D;
//		[_Newobject, "AirRaid"] call OETA_fnc_Sound3D;
    ...
    "Processing",
    false
] remoteExec ["OETA_fnc_AddHoldAction", 0, _object];
 {
