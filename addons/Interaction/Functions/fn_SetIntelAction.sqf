    ...
    "_player distance _target < 2 &&
     {
         ...
         (((group _player) getVariable ['OETA_section','Command']) in (_target getVariable ['RscAttributeOwners',['Command']]))
     }",
    "true", { [format["<t color='#FFBB00' size = '.5'>You're collecting %1.</t>",(_this select 2) select 0],-1,0.8,5,2,0,789] spawn BIS_fnc_dynamicText;}, {}, {
        _this spawn OETA_fnc_RunIntelAction;
