...
if (_type == 28) then {
        ...
        GVAR(respawnGuiParameters) params ["_faction", "_groupName", "_markerType", "_markerColor", "_markerName"];
 {
        ...
        // Close the Map
        ((findDisplay IDC_OETA_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_OETA_ADMINMENU_RESP_MAP_CONTROL) ctrlShow false;
        ((findDisplay IDC_OETA_ADMINMENU_RESP_MAP_DISPLAY) displayCtrl IDC_OETA_ADMINMENU_RESP_MAP_CONTROL) mapCenterOnCamera false;
        closeDialog IDC_OETA_ADMINMENU_RESP_MAP_DISPLAY;
    };
};
