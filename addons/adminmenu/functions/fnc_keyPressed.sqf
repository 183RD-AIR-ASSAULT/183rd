...
if (_handleKeypress) then {
    if (_authorized) then {
        if (dialog && !isNull (uiNamespace getVariable [QGVAR(display), displayNull])) then {
            systemChat "[OETA Admin Menu] The admin menu is already open"
        } else {
            if (!isNull (findDisplay 312)) then {
                systemChat "[OETA Admin Menu] Can't open the admin menu in the Zeus interface";
            } else {
        ...
        if (_authorized) then {
            if (isNull (findDisplay 5454)) then {
                if (isNull (missionNamespace getVariable ["bis_fnc_moduleRemoteControl_unit", objNull])) then {
                    systemChat "[OETA Admin Menu] Remote Control is available only through OETA Spectator";
                } else {
                    if (player isKindOf QEGVAR(spectator,unit)) then {
                        systemChat "[OETA Admin Menu] Use scroll menu action to stop remote control";
                    };
                };
            } else {
                if (isNil QEGVAR(spectator,target)) then {
                    systemChat "[OETA Admin Menu] No unit selected for Remote Control.";
                } else {
...
if (_handleKeypress && !_authorized) then {
    systemChat "[OETA Admin Menu] You're not authorized to use the admin menu";
};
