...
private _presetName = format ["oeta_preset%1", _network];
...
if (_network > -1 && !(_presetName isEqualTo _oldPresetName)) then { {
...
};
 {
    if (player canAdd _radio) then {
        ...
        systemChat format ["[183rd] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[183rd] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[183rd] Couldn't add radio: %1", _radio];
        };
