...

{
    if (player canAdd _radio) then {
        ...
        systemChat format ["[OETA] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[OETA] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[OETA] Couldn't add radio: %1", _radio];
        };
