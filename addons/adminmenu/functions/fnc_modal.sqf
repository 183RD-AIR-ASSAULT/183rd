...
if (isNil _utilityFunction) exitWith {
    systemChat format ["[OETA Admin Menu] Modal utility with name '%1' requires undefined function '%2'", _utilityName, _utilityFunction];
};
...
if (_exit) exitWith {
    systemChat "[OETA Admin Menu] No valid player(s) selected for the action!";
};
