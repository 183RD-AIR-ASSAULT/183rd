...
_ctrlButtonPreview ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[OETA Admin Menu] Message can't be empty";
    } else {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            if (_venue isEqualTo "hint") then {
                ...
                systemChat format ["[OETA Admin Message] %1", _editText];
            };
        };

        systemChat "[OETA Admin Menu] Message previewed";
    };
}];
...
_ctrlButtonCommit ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            (format ["[OETA Admin Message] %1", _editText]) remoteExec [_venue, GVAR(utilityData)];
        };

        systemChat "[OETA Admin Menu] Message sent to player(s)";
