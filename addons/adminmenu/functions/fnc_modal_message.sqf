...
private _ctrlEditPos = [0.1 * OETA_ADMINMENU_STD_WIDTH, 0.1 * OETA_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth - (0.2 * OETA_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - (1.3 * OETA_ADMINMENU_STD_HEIGHT)];
...
_ctrlCombo ctrlSetPosition [0.1 * OETA_ADMINMENU_STD_WIDTH, _ctrlGrpHeight - OETA_ADMINMENU_STD_HEIGHT, _ctrlGrpWidth * 0.3, OETA_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlSetPosition [(_ctrlGrpWidth * 0.7) - (0.2 * OETA_ADMINMENU_STD_WIDTH), _ctrlGrpHeight - OETA_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15), OETA_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonPreview ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        systemChat "[183rd Admin Menu] Message can't be empty";
    } else {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            if (_venue isEqualTo "hint") then {
                ...
                systemChat format ["[183rd Admin Message] %1", _editText];
            };
        };

        systemChat "[183rd Admin Menu] Message previewed";
    };
}];

private _ctrlButtonCommit = _display ctrlCreate [QGVAR(RscButtonMenu), -1, _ctrlGroup];
_ctrlButtonCommit ctrlSetPosition [(_ctrlGrpWidth * 0.85), _ctrlGrpHeight - OETA_ADMINMENU_STD_HEIGHT, (_ctrlGrpWidth * 0.15) - (0.1 * OETA_ADMINMENU_STD_WIDTH), OETA_ADMINMENU_STD_HEIGHT];
...
_ctrlButtonCommit ctrlAddEventHandler ["buttonClick", {
    ...
    if (_editText isEqualTo "") then {
        ...
        if (_venue isEqualTo QFUNC(showSubtitle)) then {
            ...
            (format ["[183rd Admin Message] %1", _editText]) remoteExec [_venue, GVAR(utilityData)];
        };

        systemChat "[183rd Admin Menu] Message sent to player(s)";
