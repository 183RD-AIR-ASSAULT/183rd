...
/* ----------------------------------------------------------------------------
Function: OETA_adminmenu_fnc_log
...
Parameters:
    ...
    _tag - Tag that message will be prefixed with [String, defaults to "[OETA Log]"]
...
Examples:
    (begin example)
        ["Everything is on fire!",true,"[OETA Firealarm]"] call OETA_adminmenu_fnc_log;
    (end)
    (begin example)
        [format ["%1 did something",profileName]] call OETA_adminmenu_fnc_log;
    (end)
