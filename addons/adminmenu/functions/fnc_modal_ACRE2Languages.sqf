...
_ctrlButton ctrlAddEventHandler ["ButtonClick", {
    ...
    if (count _langsToAdd == 0) then {
        if (count _langsToRemove == 0) exitWith {
            systemChat "[OETA Admin Menu] No changes selected";
        };

        systemChat "[OETA Admin Menu] Any change that would leave players with no language to speak will fail";
    };

    [_langsToAdd, _langsToRemove] remoteExecCall [QFUNC(modal_ACRE2Languages_assign), GVAR(utilityData)];

    systemChat format ["[OETA Admin Menu] Assigned ACRE2 languages to %1 player(s)", count GVAR(utilityData)];
