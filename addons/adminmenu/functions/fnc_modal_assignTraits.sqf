...
_ctrlLabelTraits ctrlSetPosition [0, 0, (0.25 * _ctrlGrpWidth), OETA_ADMINMENU_STD_HEIGHT];
...
private _ctrlComboTraitY = 2.2 * OETA_ADMINMENU_STD_HEIGHT;
private _ctrlComboTraitW = 0.5 * _ctrlGrpWidth;
private _traitComboCtrls = [];
 {
    private _ctrlLineY = _ctrlComboTraitY + _forEachIndex * (1.1 * OETA_ADMINMENU_STD_HEIGHT);
    ...
    _ctrlComboTraitLabel ctrlSetPosition [0.1 * OETA_ADMINMENU_STD_WIDTH, _ctrlLineY, _ctrlComboTraitLabelW, OETA_ADMINMENU_STD_HEIGHT];
    ...
    _ctrlComboTrait ctrlSetPosition [_ctrlComboTraitX, _ctrlLineY, _ctrlComboTraitW, OETA_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlAddEventHandler ["ButtonClick", {
    ...
    private _valueUAV = _comboUAV lbValue lbCurSel _comboUAV;
 {
    ...
    systemChat format ["[183rd Admin Menu] Assigned traits to %1 player(s)", count GVAR(utilityData)];
