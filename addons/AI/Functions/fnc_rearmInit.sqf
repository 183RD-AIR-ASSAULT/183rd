...
if(_activated) then {
    [{
        ...
        if((getpos player) inArea ([getpos _logic] + (_logic getVariable ["objectArea",[]])) && (player getVariable [QGVAR(rearmHandle),-1]) == -1) then {
            _handle = player addAction ["<t color='#FF0000'>Rearm</t>",{_this call oeta_ai_fnc_rearm},[],6,true,true,"","vehicle player != player && local (vehicle player) && !isEngineOn (vehicle player) && !(player getVariable ['oeta_ai_rearming', false])"];
