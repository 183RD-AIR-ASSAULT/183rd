...
[_mainGroup,QGVAR(garrisonGroup),true] call oeta_core_fnc_initGroupVar;
...
if (_aiNumberToSpawn > _freeBuildingSpaces) then {
    systemChat "183rd Garrison - Insufficent free building positions check your map for location and details.";
    private _mkr = createMarkerLocal [str (random 99999),getPos _logic];
    _mkr setMarkerShape "ICON";
    _mkr setMarkerType "mil_dot";
    _mkr setMarkerSize [0.5,0.5];
    _mkr setMarkerColor "ColorRed";
    _mkr setMarkerText format ["Error - OETA Garrison Module - Unable to find sufficent building places (%1 available / %2 needed)", _freeBuildingSpaces, _aiNumberToSpawn];
};
