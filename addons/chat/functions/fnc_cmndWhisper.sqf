...
/*
 * Name = OETA_chat_fnc_cmndWhisper
 * Author = Freddo
 *
 * Syntaxes:
...
if (_arg isEqualTo "") exitWith {
    systemChat "183rd Error: No argument passed. Command usage: #whisper <player> <message>";
};
...
if (_message isEqualTo "") exitWith {
    systemChat "183rd Error: No message passed. Command usage: #whisper <player> <message>";
};
...
if (!isNull _unit) then {
    ...
    systemchat format ["183rd: Whisper sent to %1", name _unit];
} else {
    systemChat FORMAT_1("183rd Error: No unit found containing %1, or more than one found.", str _message);
};
