...
missionNamespace setVariable ["oeta_straight_lines_MarkerID",100000];
...
_nul = [] spawn {
  ...
  colors = ["colorBlack","colorRed","colorYellow","colorGreen","colorBlue","colorWhite","colorBLUFOR","colorOPFOR","colorIndependent","colorCivilian","colorUNKNOWN"];
    _handle = (findDisplay 12 displayCtrl 1090) ctrlSetEventHandler ["LBSelChanged",
            "player setVariable ['oeta_straight_lines_myColor',colors select (lbCurSel (_this select 0))];
      false"
   ]
};
_nul1 = [] spawn {
  ...
  findDisplay 12 displayaddEventHandler ["MouseButtonDown",
          {if (_this select 6) then
              {
                      player setVariable ["oeta_straight_lines_myStartCoordinates",((findDisplay 12) displayCtrl 51) ctrlMapScreenToWorld [(_this select 2), (_this select 3)]];
                      player setVariable ["oeta_straight_lines_myActualCoordinates",(player getVariable "oeta_straight_lines_myStartCoordinates")];


                      _id_MouseMoving = ((findDisplay 12) displayCtrl 51) ctrladdEventHandler
                          ["MouseMoving",
                              {player setVariable ["oeta_straight_lines_myActualCoordinates", ((findDisplay 12) displayCtrl 51) ctrlMapScreenToWorld [(_this select 1),(_this select 2)]];
                              _meters = round ((player getVariable "oeta_straight_lines_myActualCoordinates") distance2D (player getVariable "oeta_straight_lines_myStartCoordinates"));
                              _azimuth = round ((player getVariable "oeta_straight_lines_myStartCoordinates") getDir (player getVariable "oeta_straight_lines_myActualCoordinates"));
                      ...
                      _id_Draw = findDisplay 12 displayCtrl 51 ctrlAddEventHandler ["Draw",
                          {
                              ...
                              [
                                  (player getVariable "oeta_straight_lines_myStartCoordinates"), (player getVariable "oeta_straight_lines_myActualCoordinates"), [1,0,0,1]
                              ];
                          }
                      ];

                      player setVariable ["oeta_straight_lines_id_MouseMoving",_id_MouseMoving];
                      player setVariable ["oeta_straight_lines_id_Draw",_id_Draw];
              }
          }
      ];
};
...
_nul2 = [] spawn {
  ...
  findDisplay 12 displayaddEventHandler
      ["MouseButtonUp",
          {if ((_this select 6) && (isnil str (player getVariable "oeta_straight_lines_myStartCoordinates"))) then
              {
                  _id_MouseMoving = player getVariable "oeta_straight_lines_id_MouseMoving";
                  _id_Draw = player getVariable "oeta_straight_lines_id_Draw";
          _myStartCoordinates = player getVariable "oeta_straight_lines_myStartCoordinates";
          _myActualCoordinates = player getVariable "oeta_straight_lines_myActualCoordinates";
          ...
          _lastMarkerID = missionNamespace getVariable "oeta_straight_lines_MarkerID";
          _newMarkerID = _lastMarkerID + 1;
          missionNamespace setVariable ["oeta_straight_lines_MarkerID",_newMarkerID];

          _myMarkerName = "_USER_DEFINED #" + str clientOwner + "/" + str _newMarkerID + "/" + str currentChannel;
                  _myColor = player getVariable ["oeta_straight_lines_myColor","colorBlack"];
          ...
          _myMarkerName setMarkerColor _myColor;
                  ...
                  player setVariable ["oeta_straight_lines_myStartCoordinates",nil];
                  player setVariable ["oeta_straight_lines_myActualCoordinates",nil];
              }
        else
        {
          _id_MouseMoving = player getVariable "oeta_straight_lines_id_MouseMoving";
