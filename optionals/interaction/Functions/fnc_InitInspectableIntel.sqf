...
/*
Function: derp_fnc_AIOccupyBuilding initInspectableIntel
...
Examples:
    ...
    ] call FUNC(initInspectableIntel);
    (end)
...
// Add the action
[_object, _actionTitle, _diaryRecord select 0] remoteExec ["FUNC(SetIntelAction)",0,true];
...
if (_notifySide) then {
    [
        ...
        {
            params[ "", "_foundBy" ];
            [format ["Intel collected by %1", name _foundBy], [1, -0.2], "#339900", 0.5] call FUNC(dynamicText);
        }
