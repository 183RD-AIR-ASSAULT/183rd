...
/*
Function: derp_fnc_AIOccupyBuilding oeta_ai_fnc_SpawnAI
...
_garrisons params ["_GarrisonedGroupsMin", ["_GarrisonedGroupsMax", 0], ["_infSkill", "OETA Default"]];
_inf params ["_infMin", ["_infMax", 0], ["_infSkill", "OETA Default"]];
_infaa params ["_infaaMin", ["_infaaMax",0], ["_infaaSkill", "OETA Default"]];
_infat params ["_infatMin", ["_infatMax",0], ["_infatSkill", "OETA Default"]];
_sniper params ["_sniperMin", ["_sniperMax",0], ["_sniperSkill", "OETA Default"]];
_vehaa params ["_vehaaMin", ["_vehaaMax",0], ["_vehaaSkill", "OETA Default"]];
_vehmrap params ["_vehmrapMin", ["_vehmrapMax",0], ["_vehmrapSkill", "OETA Default"]];
_vehlight params ["_vehlightMin", ["_vehlightMax",0], ["_vehlightSkill", "OETA Default"]];
_vehheavy params ["_vehheavyMin", ["_vehheavyMax",0], ["_vehheavySkill", "OETA Default"]];
_vehrand params ["_vehrandMin", ["_vehrandMax",0], ["_vehrandSkill", "OETA Default"]];
...
switch (_patrolMethod) do {
    case "ROAD": {
        _fnc_patrol_EI = compile "[(_this select 0), (_this select 3)] call FUNC(RoadPatrol);";
        _fnc_patrol_EI_spec = compile "[(_this select 0), (_this select 3)] call FUNC(RoadPatrol);";
        _fnc_patrol_veh = compile "[(_this select 0), (_this select 3)] call FUNC(RoadPatrol);";
    ...
    case "RANDOM": {
        ...
        _fnc_pos_EI = compile "[[[(_this select 0), (_this select 1)],[]],[""water""]] call EFUNC(common,SafePos);";
        _fnc_pos_veh = compile "[[[(_this select 0), (_this select 1)], []], [""water""], { !(_this isFlatEmpty [2,-1,0.5,1,0,false,objNull] isEqualTo []) }] call EFUNC(common,SafePos);";
    };
    default {
        _fnc_patrol_EI = compile "systemChat ""Error: Unknown patrol method supplied to oeta_ai_fnc_SpawnAI!"";";
        _fnc_patrol_EI_spec = compile "systemChat ""Error: Unknown patrol method supplied to oeta_ai_fnc_SpawnAI!"";";
        _fnc_patrol_veh = compile "systemChat ""Error: Unknown patrol method supplied to oeta_ai_fnc_SpawnAI!"";";
        _fnc_pos_EI = compile "systemChat ""Error: Unknown patrol method supplied to oeta_ai_fnc_SpawnAI!"";";
        _fnc_pos_veh = compile "systemChat ""Error: Unknown patrol method supplied to oeta_ai_fnc_SpawnAI!"";";
    };
};
...
_GetFactionArrays = compileFinal format ["call oeta_ai_fnc_%1", _faction];
...
if (_patrolMethod isEqualTo "ROAD") then {
    ...
    if (count _roadList < _minPositions) then {
        while {count _roadList < _minPositions} do {
            _rpos = [[[_center, _radius],[]],["water"]] call EFUNC(common,SafePos);
...
if !(_infList isEqualTo []) then {
   private _InfDif = _infMax - _infMin;
    ...
    for "_x" from 1 to (_infMin + floor(random _InfDif)) do {
        ...
        [_g, _infSkill] call FUNC(SetUnitSkill);
...
if !(_infaaList isEqualTo []) then {
   private _InfaaDif = _InfaaMax - _InfaaMin;
    ...
    for "_x" from 1 to (_InfaaMin + floor(random _InfaaDif)) do {
        ...
        [_g, _infaaSkill] call FUNC(SetUnitSkill);
        _units append (units _g);
    };
} else {
...
if !(_infatList isEqualTo []) then {
   private _InfatDif = _InfatMax - _InfatMin;
    ...
    for "_x" from 1 to (_InfatMin + floor(random _InfatDif)) do {
        ...
        [_g, _center, _radius, _waypoints] call _fnc_patrol_EI_spec;
        [_g, _infatSkill] call FUNC(SetUnitSkill);
        _units append (units _g);
    };
} else {
...
if !(_sniperList isEqualTo []) then {
   private _SniperDif = _SniperMax - _SniperMin;
    ...
    for "_x" from 1 to (_SniperMin + floor(random _SniperDif)) do {
        ...
        [_g, _sniperSkill] call FUNC(SetUnitSkill);
        _units append (units _g);
    };
} else {
   ...
   private _VehAADif = _VehAAMax - _VehAAMin;
    ...
    for "_x" from 1 to (_VehAAMin + floor(random _VehAADif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_g, _vehaaSkill] call FUNC(SetUnitSkill);
   ...
   private _VehMRAPDif = _VehMRAPMax - _VehMRAPMin;
    ...
    for "_x" from 1 to (_VehMRAPMin + floor(random _VehMRAPDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehmrapSkill] call FUNC(SetUnitSkill);
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
   ...
   private _VehLightDif = _VehLightMax - _VehLightMin;
    ...
    for "_x" from 1 to (_VehLightMin + floor(random _VehLightDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehLightSkill] call FUNC(SetUnitSkill);
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
   ...
   private _VehHeavyDif = _VehHeavyMax - _VehHeavyMin;
    ...
    for "_x" from 1 to (_VehHeavyMin + floor(random _VehHeavyDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehHeavySkill] call FUNC(SetUnitSkill);
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
   ...
   private _VehRandDif = _VehRandMax - _VehRandMin;
    ...
    for "_x" from 1 to (_VehRandMin + floor(random _VehRandDif)) do {
        ...
        if !(_rpos isEqualTo [0,0]) then {
            ...
            [_v, _g] call BIS_fnc_spawnCrew;
            [_g, _center, _radius, _waypoints] call _fnc_patrol_veh;
            [_g, _vehrandSkill] call FUNC(SetUnitSkill);
            if (random 1 >= 0.5) then { _v allowCrewInImmobile true; };
