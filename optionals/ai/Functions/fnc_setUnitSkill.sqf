...
/*
Function: derp_fnc_AIOccupyBuilding oeta_ai_fnc_SetUnitSkill
...
if !(isNil "OETA_AI_AimingAccuracy") then {
    _skillv = [
        (OETA_AI_AimingAccuracy - 0.05 + random 0.1),
        (OETA_AI_AimingShake - 0.05 + random 0.1),
        (OETA_AI_AimingSpeed - 0.05 + random 0.1),
        (OETA_AI_Commanding),
        (OETA_AI_Courage), 1,
        (OETA_AI_General),
        (OETA_AI_ReloadSpeed - 0.1 + random 0.30),
        (OETA_AI_SpotDistance - 0.1 + random 0.15),
        (OETA_AI_SpotTime - 0.1 + random 0.15)
    ];
};
...
if (side _target == civilian) then {
    {_x setSkill [courage, OETA_AI_CivCourage]; true } count _units;

    if (OETA_AI_CivFleeing) then {
        {
