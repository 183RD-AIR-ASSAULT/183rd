...
/*
Function: derp_fnc_AIOccupyBuilding makeIED
...
Description:
    ...
    IEDVehicle for a vanilla alternative, restricted to land vehicles.
...
if (hasInterface && {
    ...
    [
        ...
        _condition,
        "true",
        {["You've started disarming the IED.", [-1, 0.8], "#339900", 0.5, false] call EFUNC(interaction,dynamicText);},
        {["Disarming the IED.", [-1, 0.8], "#339900", 0.5, false] call EFUNC(interaction,dynamicText);},
        {
            (_this select 0) setVariable ["IEDdisarmed", true, true];
            ["You've disarmed the IED.", [-1, 0.8], "#339900", 0.5, false] call EFUNC(interaction,dynamicText);
        },
        {["The IED was not disarmed.", [-1, 0.8], "#339900", 0.5, false] call EFUNC(interaction,dynamicText);},
        [],
        10 + random 10,
        true,
        "Disarming IED",
...

        {
            if ((_object distance2D _x) < _proximityRadius && (not _disarmed) && (not _armed)) then {
                ...
                if (_announce) then {
                    ...
                    ] call EFUNC(interaction,dynamicText);
                };
            };
        } forEach allPlayers;
    },
            ...
            [
                {
                    ...
                    // Detonate using SpawnExplosives.
                    [
                        0,
                        _pos,
                        _secArray
                    ] call FUNC(SpawnExplosives);
