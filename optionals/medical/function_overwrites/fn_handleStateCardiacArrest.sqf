...
if (_timeDiff >= 1) then {
    ...
    private _totalTimeLeft = _unit getVariable ["OETA_REVIVAL", OETA_medical_persistantReviveTimer];
    if(OETA_medical_persistantRevive) then {
    ...
    _unit setVariable ["OETA_REVIVAL", _totalTimeLeft, true];
};
