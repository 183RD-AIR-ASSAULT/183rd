...
_counter = _unit getVariable ["OETA_REVIVAL",OETA_medical_persistantReviveTimer];
...
if(_active == true) then {
    _unit setVariable["OETA_revival_last_updated",CBA_missionTime];
};
if(_active == false) then {
    _lastUpdate = _unit getVariable["OETA_revival_last_updated",CBA_missionTime];
    _timeRemoved = CBA_missionTime - _lastUpdate;
    _timeLeft = _counter - _timeRemoved;
    _unit setVariable ["OETA_REVIVAL",_timeLeft];
    _unit setVariable["OETA_revival_last_updated",CBA_missionTime];
};
