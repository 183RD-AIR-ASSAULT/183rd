...
switch (OETA_Main_MaydayAccess) do {
    case 0: { _fnc_checkAccess = {false}; };
    case 1: { _fnc_checkAccess = {(_this getVariable ["OETA_Section", -1]) isEqualTo "909 EAW"}; };
        ...
        if (_vehicle isKindOf "Helicopter") then {
            ...
            if (_unit call _fnc_checkAccess) then {
                ...
                private _handle = [ {
                        ...
                        if ([_vehicle] call OETA_fnc_isPilotDown) exitWith {
                            ...
                            if ((round (speed _vehicle) != 0) || (not isTouchingGround _vehicle)) then {
                                ...
                                if (_unit in ([_vehicle] call OETA_fnc_getCopilots)) then {
                ...
                _vehicle setVariable ["OETA_MaydayHandle", _handle, false];
            };
        };
...
player addEventHandler [
    "SeatSwitchedMan", {
        ...
        if ((_vehicle isKindOf "Air") && !(_vehicle isKindOf "ParachuteBase")) then {
            if ((_playerPos isEqualTo "driver") && !(player getUnitTrait "Pilot")) exitwith {
                 [ format [hint_OETA_default, "Unlicensed Pilot has taken control of an aircraft", _unit1] ] call oeta_fnc_hint;
                // moveOut _unit1;
                // playSound "Denied";
            };
        };
...
player addEventHandler ["GetOutMan", {
    ...
    _handle = _vehicle getVariable ["OETA_MaydayHandle", -1];
